function swb_case3(wa, IWS, pEc, pEs, s_tem, s_vod, soilpar, pftpar, wet, zm, zgw)
  ...
  # Transpiration from unsaturated and saturated zones in layer #3
  ...
  wa2 = min(max((wa2 * d2 + f1 - f2 - Tr2) / d2, 0), θ_sat)
  ...
  # Layer #3
  Tr3_u = max(Tr3_u, 0)
  Tr3_u = min(Tr3_u, d3 * (wa3_unsat - wwp))

  f3 = soil_drainage(wa3_unsat, θ_sat, ks, 0.012, 1.2)
  wa3_unsat = min(max((wa3_unsat * d3 + f2 + ff2 - f3 - Tr3_u) / d3, 0), θ_sat)

  ff3 = max((wa3_unsat - θ_sat) * d3, 0)
  wa3_unsat = min(wa3_unsat, θ_sat)
